...
def check_searched_name(user_name):
    ...
    if notexist:
       ...
       with open('/Users/Paul/desktop/enhancedstreaming/searched_list.txt','a') as f:
           f.write(user_name + "\n")
           return True
    else:
        return False

# def add_postcode_suburb():
...
class TwitterStreamListener(tweepy.StreamListener):
    ...
    def on_status(self, status):
        if check_requirements(status._json) == True:
            ...
            if check_searched_name(username) == True:
...
if __name__ == '__main__':
    ...
    try:
        data_base = couch.create('try')
    except couchdb.http.PreconditionFailed as e:
        data_base = couch['try']
    
    # read the searched list
    with open('/Users/Paul/desktop/enhancedstreaming/searched_list.txt','r') as f:
